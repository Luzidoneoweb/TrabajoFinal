...
if (isset($_GET['obtener_conteo_palabras']) && isset($_GET['text_id'])) {
    $text_id = intval($_GET['text_id']);
    
    $stmt = $conn->prepare("SELECT COUNT(*) as conteo_palabras FROM saved_words WHERE user_id = ? AND text_id = ?");
    $stmt->bind_param("ii", $user_id, $text_id);
    $stmt->execute();
    $result = $stmt->get_result();
    $data = $result->fetch_assoc();
    
    echo json_encode(['conteo_palabras' => $data['conteo_palabras']]);
    $stmt->close();
    $conn->close();
    exit();
}
